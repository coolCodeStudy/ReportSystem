...
| 2026-07-10 | Linux LibreOffice substituted the DOCX `华文宋体` body font with Noto Sans CJK/DejaVu when exporting PDF. | PDF typography looked visibly different from the Word/WPS document even when layout was otherwise correct. | `scripts/test_deploy_workflow.py` requires serif CJK packages and a fontconfig alias mapping `华文宋体`/`STSong`/`Songti SC` to Noto Serif CJK for server PDF conversion. | This commit |
